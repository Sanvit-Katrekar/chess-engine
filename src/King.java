...
public class King extends Piece{
    ...
    public King(String pos, boolean isWhite,String n, LinkedList<Piece> ps){
        super(pos, isWhite, n, ps);
    }

    public static void main(String[] args) {
        while (!"A".equals(ps.get(kingPositionC/8).get(kingPositionC%8))) {kingPositionC++;}//get King's location
        while (!"a".equals(ps[kingPositionL/8][kingPositionL%8])) {kingPositionL++;}//get king's location
    ...
    public static String posibleA(int i) {
        ...
        for (int j=0; j<9; j++) {
            if (j!=4) {
                try {
                    if (Character.isLowerCase(ps[r-1+j/3][c-1+j%3].charAt(0)) || " ".equals(ps[r-1+j/3][c-1+j%3])) {
                        oldPiece=ps[r-1+j/3][c-1+j%3];
                        ps[r][c]=" ";
                        ps[r-1+j/3][c-1+j%3]="A";
